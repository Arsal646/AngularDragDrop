...
export class HomeComponent implements OnInit {
  todo: any = []
	...
	constructor() {
	  let item = ['Get to work', 'Pick up groceries', 'Go home', 'Fall asleep']
	...
	valueSet() {
	  let getSeats: any = localStorage.getItem('Seats')
	  let Seats = []
	  Seats = JSON.parse(getSeats)
	  if (Seats == null) {
		this.Tickets = [
		  {
			id: 1,
    ...
    for(let item in this.Seats){
      this.singleItemIndropZone(this.Seats)
    }
	...
	removeLastDroppedItem(main: any, item: any) {
	  ...
	  this.Seats.forEach((element: any) => {
		...
		if (element.children.length) {
		  element.children = element.children.filter((val: any) => {
			return val != item
		  })
		  console.log(element)
